...
static void log_callback(void *arg)
{
	CURL* curl;
	CURLMsg *msg;
	long respcode;
...
void log_init()
{
	...
	// Use a thread to keep stuff as in sync as possible
	logthread = schedule(LOG_DELAY_MS,log_callback,NULL);
}

void log_stop()
{
	unschedule(logthread);
	fclose(logfile);
